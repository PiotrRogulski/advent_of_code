...
class _P1 extends PartImplementation<_I, _O> {
  ...
  _O runInternal(_I inputData) => .new(
    inputData.matrix.cells
            .firstWhere((c) => c.value == .start)
            .apply(
              (c) => (
                matrix: inputData.matrix.copy()..setIndex(c.index, .empty),
    ...
    inputData.matrix.cells
        ...
        .where(
          (p) =>
              ...
              inputData.matrix.atIndex(p) != .start,
        )
...
bool hasLoop(MatrixIndex newObstacle, Matrix<_Tile> matrix) {
  var position = matrix.cells.firstWhere((c) => c.value == .start).index;
  ...
  while (true) {
    ...
    if (nextTile == .obstacle || nextPosition == newObstacle) {
